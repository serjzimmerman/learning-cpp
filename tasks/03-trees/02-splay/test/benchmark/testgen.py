...
def generate_request(p_list: np.array, p_config: dict) -> list:
    ...
    for i in range(length):
        lower = np.random.default_rng().integers(
            int(p_list[0] * 0.5), int(p_list[len(p_list) - 1] * 1.15)
        )
        upper = np.random.default_rng().integers(
            lower + 1, int(p_list[len(p_list) - 1] * 1.25)
        )
...
def generate_normal(
    ...
    for i in range(normal_dict["number"]):
        sample = list(
            set(map(int, (np.random.default_rng().normal(mean, deviation, length))))
        )
        sample.sort()
...
def generate_uniform(
    ...
    for i in range(uniform_dict["number"]):
        sample = list(
            set(map(int, np.random.default_rng().integers(lower, upper, length)))
        )
        sample.sort()
