...
private:
  ...
  auto row_pointer(size_type index) {
    return m_contiguous_matrix.data() + m_rows_vec[index];
  }
  auto row_pointer(size_type index) const {
    return m_contiguous_matrix.data() + m_rows_vec[index];
  }

public:
  proxy_row operator[](size_type index) {
    return proxy_row{row_pointer(index), cols()};
  }
  const_proxy_row operator[](size_type index) const {
    return const_proxy_row{row_pointer(index), cols()};
  }
...
public:
    ...
    requires std::is_floating_point_v<value_type>
  {
    ...
    for (size_type i = 0; i < min_dimension(); i++) {
      ...
      if (is_roughly_equal(pivot_elem, value_type{}))
        return 0;
      ...
      for (size_type to_elim_row = 0; to_elim_row < rows(); to_elim_row++) {
        ...
        std::transform(first_row.begin(), first_row.end(), second_row.begin(),
                       first_row.begin(),
  ...
  matrix &operator+=(const matrix &other) {
    ...
    for (size_type i = 0; i < rows(); ++i) {
      ...
      std::transform(row_first.begin(), row_first.end(), row_second.begin(),
                     row_first.begin(), std::plus<value_type>{});
    }
    return *this;
  }
  ...
  matrix &operator-=(const matrix &other) {
    ...
    for (size_type i = 0; i < rows(); ++i) {
      ...
      auto row_second = other[i];
      std::transform(row_first.begin(), row_first.end(), row_second.begin(),
                     row_first.begin(), std::minus<value_type>{});
    }
    return *this;
  }
