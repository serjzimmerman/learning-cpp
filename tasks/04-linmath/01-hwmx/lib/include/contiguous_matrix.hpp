...
private:
  ...
  static_assert(ranges::random_access_range<proxy_row>,
                "Proxy row is not a random access range");
  static_assert(ranges::random_access_range<const_proxy_row>,
                "Const proxy row is not a random access range");
...
public:
  ...
  contiguous_matrix &operator+=(const contiguous_matrix &other) {
    ...
    ranges::transform(m_buffer, other.m_buffer, m_buffer.begin(),
                      std::plus<value_type>{});
    return *this;
  }
  ...
  contiguous_matrix &operator*=(value_type rhs) {
    ranges::actions::transform(m_buffer,
...
public:
  ...
  using const_iterator =
      typename containers::vector<value_type>::const_iterator;
...
static_assert(ranges::random_access_range<contiguous_matrix<float>>,
              "Contigous matrix is not a random access range");
