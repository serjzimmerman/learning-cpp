...
template <typename t_base> struct visitable_base {
  ...
  static constexpr detail::unique_tag_type unique_tag_helper_ezviz__(
      const t_visitable * =
...
public:
  return_type apply(base_type &base) {
    const auto *vtable = static_cast<t_concrete_visitor &>(*this)
                             .template get_vtable_ezviz__<vtable_traits_type>();
